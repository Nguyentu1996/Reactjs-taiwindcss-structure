...
export default function Menu(props) {
    ...
    menuArray.map((item, index) => 
        menuListElement.push(
            <div key={index} style={(index === activeMenu) ? activeStyle : {}}
    ...
    return <div className="flex bg-default h-11 overflow-x-scroll hide-scroll sm:overflow-hidden">
        <div className='flex self-center cursor-pointer pr-2 md:container mx-auto sm:container mx-auto'>
            <div id="menu" className="flex absolute top-2 left-0 ml-2 xl:static m-0 md:static m-0 sm:flex static m-0" onClick={props.toogleSidebar}>
                <Icons.Menu className='text-gray-50 focus:outline-none hover:text-yellow-300 h-3/5' />
                <p className='text-gray-50 hidden text-sm self-center focus:outline-none pr-3 xl:inline-flex md:inline-flex sm:inline-flex'>All</p>
            </div>
            {menuListElement}
        </div>
    </div>
